...
- (UITableView *)buildTableView{
    ...
    tableView.separatorStyle = _allowMultipleSelection?UITableViewCellSeparatorStyleNone:UITableViewCellSeparatorStyleSingleLine;
    if (self.isNeedSingleStyle) {
        tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    }
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // 根据用户上次选择的,展示
        NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
        NSString *key = [NSString stringWithFormat:@"userDefault_%@_lastOffsetY",self.modelIndex];
        CGFloat lastOffsetY = [[userDefault objectForKey:key] floatValue];
        [tableView setContentOffset:CGPointMake(0, lastOffsetY) animated:YES];
    });
    return tableView;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        ...
        cell.textLabel.textAlignment = _allowMultipleSelection?NSTextAlignmentLeft:NSTextAlignmentCenter;
        if (self.isNeedSingleStyle) {
...
#pragma mark - UITableViewDelegate
    ...
    {
        // 保存最后滚动的位置
        // 保存contentOffset，下次直接滚动到这儿
        CGFloat yOffset = scrollView.contentOffset.y;
        // 根据用户上次选择的,展示
        NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
        [userDefault setObject:@(yOffset) forKey:[NSString stringWithFormat:@"userDefault_%@_lastOffsetY",self.modelIndex]];
